...
class ConsequentialityEngine:
    def assess(
        ...
        *,
        ...
        extras = context or {}
        if extras.get("non_negotiable_conflict") or extras.get("value_conflict"):
            level = _higher(level, ConsequentialityLevel.CRITICAL)
            reasons.append("NON_NEGOTIABLE_CONFLICT")
        if extras.get("kill_assumption") or extras.get("kill_assumption_event"):
            level = _higher(level, ConsequentialityLevel.CRITICAL)
            reasons.append("KILL_ASSUMPTION_EVENT")
        if extras.get("financial_commitment") or extras.get("irreversible_commitment"):
            level = _higher(level, ConsequentialityLevel.CRITICAL)
            reasons.append("IRREVERSIBLE_COMMITMENT")
        if extras.get("legal_consequence") or extras.get("regulatory_consequence"):
            level = _higher(level, ConsequentialityLevel.CRITICAL)
            reasons.append("LEGAL_OR_REGULATORY_CONSEQUENCE")
        if extras.get("safety_consequence") or extras.get("stakeholder_harm"):
            level = _higher(level, ConsequentialityLevel.CRITICAL)
            reasons.append("SAFETY_OR_STAKEHOLDER_HARM")
        if extras.get("evidence_uncertainty"):
        ...
        if project is not None:
            ...
            if value.get("non_negotiable_conflict") or value.get("conflict"):
                level = _higher(level, ConsequentialityLevel.CRITICAL)
                reasons.append("NON_NEGOTIABLE_CONFLICT")
